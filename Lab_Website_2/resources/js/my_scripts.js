...
function loadStatsPage(){
	...
	for(row=2;row<table.rows.length;row++){
		for(col=2;col<table.rows[row].cells.length-2;col++){
			home_score=Number(table.rows[row].cells[col].innerHTML);
			opp_score=Number(table.rows[row].cells[col+1].innerHTML);
			if(home_score>opp_score){
				table.rows[row].cells[col+2].innerHTML="CU Boulder"
				winCount++;
			}
			else{
				table.rows[row].cells[col+2].innerHTML=table.rows[row].cells[col-1].innerHTML;
				lossCount++;
			}
		}
	}
...
function loadPlayersPage(){
	var selectionText=""
	var playern=0;
	for(playern=0; playern<players.length; playern++){
		selectionText+='<a href="#" onclick="switchPlayers('+playern+')">'+players[playern].name+'</a></br>';
	}
